...
namespace MainClient
{
    class MyTcpClient
    {
        ...
        public MyTcpClient(int sendBufferSize, int receiveBufferSize)
        {
            Init(sendBufferSize, receiveBufferSize);
        }
        public void Init(int sendBufferSize, int receiveBufferSize)
        {
        ...
        private void OnConnect(IAsyncResult iar)
        {
            try
            {
                ...
                ClientNetManager.Instance.OnConnect(true);
                tcp.GetStream().BeginRead(_receiveBuffer, _currReceiveLen, _receiveBuffer.Length - _currReceiveLen, OnReceive, tcp);
                Send();
            }
            catch (Exception ex)
            {
                ...
                _state = SocketState.STATE_CLOSED;
                ClientNetManager.Instance.OnConnect(false);
                Close();
            }
        }
        ...
        private void OnSend(IAsyncResult iar)
        {
            ...
            {
                Close();
                ClientNetManager.Instance.OnClosed(NetErrorCode.NET_SYSTEERROR);
        ...
        public bool Send(Byte[] buffer, int start, int length)
        {
            ...
            if (length + 1 + usedSize > size)
            {
                ...
                Close();
                ClientNetManager.Instance.OnClosed(NetErrorCode.NET_SENDBUFF_OVERFLOW);
                return false;
            }
        ...
        private void OnReceive(IAsyncResult iar)
        {
            try
            {
                ...
                TcpClient tcp = (TcpClient)iar.AsyncState;
                int receiveSize = tcp.GetStream().EndRead(iar);
                if (receiveSize > 0)
                {
                    _TotalReceiveBytes += receiveSize;
                    _currReceiveLen += receiveSize;
                    if (_currReceiveLen >= _receiveBuffer.Length)
                    {
                        Debug.LogError("receiveBuffer is full package len = " + ClientNetManager.Instance.GetPacketLen(_receiveBuffer, 0, _currReceiveLen));
                    }
                    int usedSize = ClientNetManager.Instance.DetectPacket(_receiveBuffer, _currReceiveLen);
                    if (usedSize > 0)
                    {
                ...
                else if (0 == receiveSize)
                {
                    Debug.LogWarning("Close socket normally");
                    Close();
                    ClientNetManager.Instance.OnClosed(NetErrorCode.NET_NO_ERROR);
                }
                else
                {
